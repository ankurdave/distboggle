...
public class BoggleHillClimbClient {
    ...
    public static void main(String[] args) throws IOException {
	    ...
	    inputLine = in.readLine();
	    Boggle best = new Boggle(inputLine.split(" ")[1], 4, Integer.parseInt(inputLine.split(" ")[2]), dict);
    	best.generate();
    	System.err.println("Start boggle: " + inputLine);
		Boggle trial;
		
		for (int i = 0;; i++) {
			if (i % 1000 == 0) System.err.println("Mutation attempt " + i);
			// mutate the current boggle
			trial = best.mutate(50);
			trial.generate();
			// replace best with trial if trial is better
			if (trial.getScore() > best.getScore() || i >= 100000)
			{
				best = trial;
				System.out.println(i + " " + best.getScore());

				// send improved boggle to the server
				outputLine = clientID + " " + best.gridToString() + " " + best.getScore();
		    	out.println(outputLine); // tells server we've finished a "generation" and sends migrant
		    	System.err.println("Sent boggle: " + outputLine);
		    	
		    	// get a migrant from the server, replacing the current one with the new one
			    inputLine = in.readLine();
			    if (inputLine == null) {
			    	System.err.println("Server closed connection.");
			    	break;
			    }
			    if (!inputLine.isEmpty()) { // if the server has no migrants, keep mutating the current one
			    	best = new Boggle(inputLine.split(" ")[1], 4, Integer.parseInt(inputLine.split(" ")[2]), dict);
			    	best.generate();
